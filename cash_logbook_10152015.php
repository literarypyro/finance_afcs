...
<td>
Cash Logbook | <a href='merged_logbook.php'>SJ/SV Logbook</a> | <a href='#' onclick='window.open("dsr_cash.php","_blank")'>Detailed Sales Report</a>
</td>
<td align=right >
...
<tr class='subheader'>
	...
	<!--
	<td rowspan=2 align=right>Issuance Fee</td>
	-->
...
$row=$rs->fetch_assoc();
	...
	$sjtNetTotal=$row['sjt_net_revenue'];
	$svcNetTransfer=$row['svc_net_revenue'];
	$svcIssuanceTotal=$row['svc_issuance_fee'];


	$grandTotal=($row['sjt_net_revenue']*1)+($row['svc_net_revenue']*1)+($row['svc_issuance_fee']*1)+($row['revolving_fund']*1);
}
else {
...
$row=$rs2->fetch_assoc();
	...
	$insert="insert into beginning_balance_cash(log_id,revolving_fund,sjt_net_revenue,svc_net_revenue,svc_issuance_fee) values ('".$log_id."','".$revolvingTotal."','".$sjtNetTotal."','".$svcNetTotal."','".$svcIssuanceTotal."')";
...
<tr <?php echo $style; ?>>
	...
	if($type=="allocation"){
		...
		$svcIssuanceTotal=$svcIssuanceTotal+$svc_issuance_fee;
	}
