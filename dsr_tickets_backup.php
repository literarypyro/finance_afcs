...
for($i=0;$i<$nm;$i++){
	...
	$sql2_test="select * from control_remittance where remit_log='".$log_id."' and station='".$stationStamp."' group by remit_ticket_seller,unit";
	echo $sql2_test."<br>";
	$rs2=$db->query($sql2);
	$nm2=$rs2->num_rows;
...
for($k=0;$k<$nm2;$k++){
	...
	$ticketsellerSQL="select * from ticket_seller where id='".$row2['ticket_seller']."'";	
	...
	$discountSQL="select * from discount where control_id in (SELECT control_id FROM control_remittance where remit_log='".$log_id."' and station='".$stationStamp."' and remit_ticket_seller='".$row2['ticket_seller']."')";
	...
	$unregSQL="select * from unreg_sale where control_id in (SELECT control_id FROM control_remittance where remit_log='".$log_id."' and station='".$stationStamp."' and remit_ticket_seller='".$row2['ticket_seller']."')";
	...
	$unsoldSQL="select type,sum(loose_good) as ticket_sum,sum(loose_defective) as ticket_sum2 from control_unsold where control_id in (SELECT control_id FROM control_remittance where remit_log='".$log_id."' and station='".$stationStamp."' and remit_ticket_seller='".$row2['ticket_seller']."') group by type";
...
/*
	$unsoldSQL="select sum(sjd) as sjd,sum(sjt) as sjt,sum(svd) as svd,sum(svt) as svt from physically_defective where log_id='".$log_id."' and ticket_seller='".$row2['ticket_seller']."'";
...
*/
	$refundSQL="select * from refund where control_id in (SELECT control_id FROM control_remittance where remit_log='".$log_id."' and station='".$stationStamp."' and remit_ticket_seller='".$row2['ticket_seller']."')";
...
//	$cashSQL="select * from control_cash where control_id in (SELECT control_id FROM remittance where log_id='".$log_id."' and ticket_seller='".$row2['ticket_seller']."')";

	$cashSQL="select sum(if(discrepancy.type='overage',amount,0)) as overage,sum(if(discrepancy.type='shortage',amount,0)) as unpaid_shortage from discrepancy inner join cash_transfer on discrepancy.transaction_id=cash_transfer.transaction_id where discrepancy.log_id='".$log_id."' and discrepancy.ticket_seller='".$row2['ticket_seller']."' and cash_transfer.station='".$stationStamp."'";
	//	$cashSQL="select * from control_cash where control_id in (SELECT control_id FROM remittance where log_id='".$log_id."' and ticket_seller='".$row2['ticket_seller']."')";
	...
	$db=retrieveDb();	
	//$discrepancySQL="SELECT * FROM transaction inner join cash_transfer on transaction.transaction_id=cash_transfer.transaction_id where transaction_type='shortage' and log_id='".$log_id."' and ticket_seller='".$row2['ticket_seller']."'";
	
	$discrepancySQL="SELECT * FROM transaction inner join cash_transfer on transaction.transaction_id=cash_transfer.transaction_id where transaction_type='shortage' and transaction.log_id='".$log_id."' and cash_transfer.station='".$stationStamp."' and cash_transfer.ticket_seller='".$row2['ticket_seller']."'";
	...
	$discrepancyTicketSQL="select *,sum(amount) as new_amount from discrepancy_ticket where transaction_id in (select control_slip.id from control_slip inner join remittance on control_slip.id=remittance.control_id where remittance.log_id='".$log_id."' and control_slip.ticket_seller='".$row2['ticket_seller']."' and control_slip.station='".$stationStamp."') group by ticket_type";
//	echo $control;	

//	$discrepancyTicketSQL="select *,sum(amount) as new_amount from discrepancy_ticket inner join control_slip on discrepancy_ticket.transaction_id=concat('control_',control_slip.id) where discrepancy_ticket.log_id='".$log_id."' and discrepancy_ticket.ticket_seller='".$row2['ticket_seller']."' and control_slip.station='".$stationStamp."' group by ticket_type";
...
<th colspan=3>Subtotal</th>
	<td align=right><font color=red><?php echo number_format($subtotal['sj_unreg']*1,2); ?></font></td> 
	<td align=right><font color=red><?php echo number_format($subtotal['sv_unreg']*1,2); ?></font></td> 
	<td align=right><font color=red><?php echo number_format($subtotal['sj_discount']*1,2); ?></font></td> 
	<td align=right><font color=red><?php echo number_format($subtotal['sv_discount']*1,2); ?></font></td> 
	<td align=right><font color=red><?php echo $subtotal['sj_refund']; ?></font></td> 

	<td align=right><font color=red><?php echo number_format($subtotal['sj_r_amount']*1,2); ?></font></td> 

	<td align=right><font color=red><?php echo $subtotal['sv_refund']; ?></font></td> 

	<td align=right><font color=red><?php echo number_format($subtotal['sv_r_amount']*1,2); ?></font></td> 

	<td align=right><font color=red><?php echo number_format($subtotal['overage']*1,2); ?></font></td> 

	<td align=right><font color=red><?php echo number_format($subtotal['paid_shortage']*1,2); ?></font></td> 
	<td align=right><font color=red><?php echo number_format($subtotal['unpaid_shortage']*1,2); ?></font></td> 

	<td align=right><font color=red><?php echo $subtotal['sjtLoose']; ?></font></td> 
	<td align=right><font color=red><?php echo $subtotal['sjdLoose']; ?></font></td> 
	<td align=right><font color=red><?php echo $subtotal['svdLoose']; ?></font></td> 
	<td align=right><font color=red><?php echo $subtotal['svtLoose']; ?></font></td> 
			
	<td align=right><font color=red><?php echo $subtotal['sjt_label']; ?></font></td> 
	<td align=right><font color=red><?php echo $subtotal['sjd_label']; ?></font></td> 
	<td align=right><font color=red><?php echo $subtotal['svd_label']; ?></font></td> 
	<td align=right><font color=red><?php echo $subtotal['svt_label']; ?></font></td> 

	<td align=right><font color=red><?php echo $subtotal['sjtDefective']; ?></font></td> 
	<td align=right><font color=red><?php echo $subtotal['sjdDefective']; ?></font></td> 
	<td align=right><font color=red><?php echo $subtotal['svdDefective']; ?></font></td> 
	<td align=right><font color=red><?php echo $subtotal['svtDefective']; ?></font></td> 
	</tr>
<?php
}
}
...
<th colspan=3>Grand Total</th>
	<td align=right><font color=red><?php echo number_format($grandtotal['sj_unreg']*1,2); ?></font></td> 
	<td align=right><font color=red><?php echo number_format($grandtotal['sv_unreg']*1,2); ?></font></td> 
	<td align=right><font color=red><?php echo number_format($grandtotal['sj_discount']*1,2); ?></font></td> 
	<td align=right><font color=red><?php echo number_format($grandtotal['sv_discount']*1,2); ?></font></td> 
	<td align=right><font color=red><?php echo $grandtotal['sj_refund']; ?></font></td> 
	<td align=right><font color=red><?php echo number_format($grandtotal['sj_r_amount']*1,2); ?></font></td> 

	<td align=right><font color=red><?php echo $grandtotal['sv_refund']; ?></font></td> 

	<td align=right><font color=red><?php echo number_format($grandtotal['sv_r_amount']*1,2); ?></font></td> 

	<td align=right><font color=red><?php echo number_format($grandtotal['overage']*1,2); ?></font></td> 

	<td align=right><font color=red><?php echo number_format($grandtotal['paid_shortage']*1,2); ?></font></td> 
	<td align=right><font color=red><?php echo number_format($grandtotal['unpaid_shortage']*1,2); ?></font></td> 

	<td align=right><font color=red><?php echo $grandtotal['sjtLoose']; ?></font></td> 
	<td align=right><font color=red><?php echo $grandtotal['sjdLoose']; ?></font></td> 
	<td align=right><font color=red><?php echo $grandtotal['svdLoose']; ?></font></td> 
	<td align=right><font color=red><?php echo $grandtotal['svtLoose']; ?></font></td> 
			
	<td align=right><font color=red><?php echo $grandtotal['sjt_label']; ?></font></td> 
	<td align=right><font color=red><?php echo $grandtotal['sjd_label']; ?></font></td> 
	<td align=right><font color=red><?php echo $grandtotal['svd_label']; ?></font></td> 
	<td align=right><font color=red><?php echo $grandtotal['svt_label']; ?></font></td> 

	<td align=right><font color=red><?php echo $grandtotal['sjtDefective']; ?></font></td> 
	<td align=right><font color=red><?php echo $grandtotal['sjdDefective']; ?></font></td> 
	<td align=right><font color=red><?php echo $grandtotal['svdDefective']; ?></font></td> 
	<td align=right><font color=red><?php echo $grandtotal['svtDefective']; ?></font></td> 
	</tr>
</table>