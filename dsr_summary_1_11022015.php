...
//	$sql2="select (ot) as ot,tvm_refund from control_cash inner join control_remittance on control_cash.control_id=control_remittance.control_id where remit_log='".$log_id."' and station='".$stationStamp."' group by type";
	/*
...
*/	
	...
	$sjt_deduction=$sjt_defective+$sold_ticket_1['sjt']['reg'];	
...
<tr class='grid'>
	...
	<td style='border:1px solid gray' align=right><?php echo number_format($sold_ticket_1['sjt']['reg']*1,0); ?></td>
	<td style='border:1px solid gray' align=right><?php echo number_format($sold_ticket_1['svc']['reg']*1,0); ?></td>
</tr>	
<tr class='grid'>
	<th style='border:1px solid gray'>Remitted Loose</th>
	
	<td style='border:1px solid gray' align=right><?php echo number_format($sjt_loose*1,0); ?></td>
	<td style='border:1px solid gray' align=right><?php echo number_format($svt_loose*1,0); ?></td>
</tr>	
...
for($i=0;$i<$nm;$i++){
	...
	$sql2="select * from control_sales_amount inner join control_remittance on control_sales_amount.control_id=control_remittance.control_id where  source_type='pos' and ticket_type='svc' and remit_log='".$log_id."' and station='".$stationStamp."'";
	$rs2=$db->query($sql2);
	$nm2=$rs2->num_rows;	
	for($k=0;$k<$nm2;$k++){
		$row2=$rs2->fetch_assoc();
...
//	$sql2="select sum(ot) as ot from control_cash inner join control_remittance on control_cash.control_id=control_remittance.control_id where remit_log='".$log_id."' group by type";
	$sql2="select (ot) as ot, tvm_refund from control_cash inner join control_remittance on control_cash.control_id=control_remittance.control_id where remit_log='".$log_id."' group by type";
