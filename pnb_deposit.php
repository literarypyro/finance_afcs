...
if((isset($_POST['amount']))&&($_POST['amount']>0)){
	...
	if($_POST['form_action']=="insert"){
		...
		$sql.="transaction_id,amount,reference_id,account_type) values ";
		$sql.="('".$log_id."','".$date."','".$_POST['cash_assistant']."','".$type."',";
		$sql.="'".$transaction_id."','".$total."','".$reference_id."','".$account_type."')";
...
<tr class='category'>
	...
	<td>
		<select name='account_type'>
			<option value='sjt' <?php if($account_type=="sjt"){ echo "selected"; } ?>>SJT</option>
			<option value='svc' <?php if($account_type=="svc"){ echo "selected"; } ?>>SVC</option>
		</select>
	</td>
</tr>
<tr class='category'>
