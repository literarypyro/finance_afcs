...
if((isset($_POST['cash_total']))&&($_POST['cash_total']>0)){
	...
	$svc_issuance_fee=$_POST['svc_issuance_fee'];
	...
	if($_POST['form_action']=="insert"){
		...
		$sql.="transaction_id,total_in_words,total,sjt_net_revenue,svc_net_revenue,svc_issuance_fee,station,reference_id,unit,destination_ca) values ";
		$sql.="('".$log_id."','".$date."','".$ticket_seller."','".$_POST['cash_assistant']."','".$type."',";
		$sql.="'".$transaction_id."','".$totalWords."','".$revolving."','".$sjt_net_revenue."','".$svc_net_revenue."','".$svc_issuance_fee."','".$station_entry."','".$reference_id."','".$unit."','".$destination_ca."')";
		...
		if(isset($_POST['trans_edit'])){
			...
			$sql="update cash_transfer set ticket_seller='".$ticket_seller."',total='".$revolving."',sjt_net_revenue='".$sjt_net_revenue."',svc_net_revenue='".$svc_net_revenue."',svc_issuance_fee='".$svc_issuance_fee."',total_in_words='".$totalWords."',station='".$station_entry."',type='".$type."',unit='".$unit."', destination_ca='".$destination_ca."',reference_id='".$reference_id."' where transaction_id='".$transaction_id."'";
			$rs=$db->query($sql);
			//			echo $sql;
			//			$sql="insert into cash_transfer(log_id,time,ticket_seller,cash_assistant,type,";
...
<tr><td  class='subheader'>Revolving Fund (Remittance)</td><td class='grid' colspan=2><input style='text-align:right'  onkeyup='verifyTotal()' type=text id='revolving_remittance' name='revolving_remittance' value='<?php echo $revolvingpost; ?>'   /></td></tr>
<tr><td class='subheader'>For Deposit/Net Revenue (SJT)</td><td class='category' colspan=2><input style='text-align:right'  onkeyup='verifyTotal()'  type=text name='sjt_net_revenue' id='sjt_net_revenue' value='<?php echo $sjt_net_revenue; ?>' /></td></tr>
<tr><td class='subheader'>For Deposit/Net Revenue (SVC)</td><td class='category' colspan=2><input style='text-align:right'   onkeyup='verifyTotal()' type=text name='svc_net_revenue' id='svc_net_revenue' value='<?php echo $svc_net_revenue; ?>' /></td></tr>
...
function verifyTotal(){
	var verify_total=document.getElementById('revolving_remittance').value+document.getElementById('sjt_net_revenue').value+document.getElementById('svc_net_revenue').value;
	document.getElementById('verify_total').value=verify_total;
