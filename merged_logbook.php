...
<td>
<a href="cash_logbook.php" >Cash Logbook</a> | SJ/SV Logbook | <a href='#' onclick='window.open("dsr_cash.php","_blank")'>Detailed Sales Report</a>
</td>
...
<td align=center colspan=2>SVC</td>
...
for($a=0;$a<$nm;$a++){
	...
	if(($row['log_type']=='ticket')||($row['log_type']=="afpi")||($row['log_type']=="annex")||($row['log_type']=="finance")){
		...
		$svt_packs=$ticketRow['svt']*1+$ticketRow['c']*1;
...
//		$sjd_packs=($ticketRow['sjd']*1-$sjd_loose);	
		...
		if($trans_type=="allocation"){
			$ticketSQL="select * from allocation where transaction_id='".$transaction_id."' and type in ('sjd','sjt','svd','svt')";
			...
			$looseSQL="select * from control_unsold where transaction_id='".$transaction_id."' and type in ('sjd','sjt','svd','svt')";
...
<tr>
	...
	if($log_type=="initial"){
		if($_SESSION['viewMode']=="login"){
			echo "<a href='javascript:void(0)' style='text-decoration:none' onclick='window.open(\"control_slip.php?edit_control=".$control_id."\",\"control slip\",\"height=750, width=800, scrollbars=yes\")'>".strtoupper($ticketSellerRow['last_name']).", ".$ticketSellerRow['first_name']; 
		...
		if($_SESSION['viewMode']=="login"){
			echo "<a href='javascript:void(0)' style='text-decoration:none' onclick='window.open(\"ticket_order.php?tID=".$edit_id."\",\"transfer\",\"height=420, width=500, scrollbars=yes\")'>FROM ANNEX</a>"; 
		}
		else {
			echo "FROM ANNEX";
		}
		...
		if($_SESSION['viewMode']=="login"){
			echo "<a href='javascript:void(0)' style='text-decoration:none' onclick='window.open(\"ticket_order.php?tID=".$edit_id."\",\"transfer\",\"height=420, width=500, scrollbars=yes\")'>FROM AFPI</a>"; 
		}
		else {
			echo "FROM AFPI";
		}
		...
		if($_SESSION['viewMode']=="login"){
			echo "<a href='javascript:void(0)' style='text-decoration:none' onclick='window.open(\"ticket_order.php?tID=".$edit_id."\",\"transfer\",\"height=420, width=500, scrollbars=yes\")'>FROM FINANCE TRAIN</a>"; 
		}
		else {
		...
		if($_SESSION['viewMode']=="login"){
			echo "<a href='javascript:void(0)' style='text-decoration:none' onclick='window.open(\"ticket_order.php?tID=".$edit_id."\",\"transfer\",\"height=420, width=500, scrollbars=yes\")'>".strtoupper($ticketSellerRow['last_name']).", ".$ticketSellerRow['first_name'].$suffix; 
...
<?php
$verify=$sjt_packs_1+$sjt_loose_1+$sjd_packs_1+$sjd_loose_1+$svt_packs_1+$svt_loose_1+$svd_packs_1+$svd_loose_1;	
...
if($verify==0){
	...
	$transferRS2=$db->query($transferBalance);		
...
<tr>
<td>
<a href='generateMergedLogbook.php' target='_blank'>Generate Printout</a>
